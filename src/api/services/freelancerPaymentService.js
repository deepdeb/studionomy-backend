...
exports.getFreelancerPayment = async (req, res) => {
    try {
        let sql = "SELECT rb.req_to, rb.job_id, rb.job_number, f.freelancerName, j.job_startDate, j.job_endDate, j.job_details, j.total_amount, rb.payment FROM request_for_booking rb JOIN jobs j ON rb.job_id = j.job_id JOIN freelancer f ON f.freelancer_id = rb.req_to WHERE rb.req_status = 'accepted' AND rb.isDeleted = 0 AND rb.isCancel = 0 AND rb.req_from = ? AND rb.req_from_userType = ?"
