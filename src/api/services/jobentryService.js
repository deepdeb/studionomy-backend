...
exports.jobentry = async (data) => {
    ...
    try {
        if (!data.job_id) {
            ...
            //     jobsUsedUpdateFromMainResp = await connection.query(jobsUsedUpdateFromMainSql, [data.userId, data.userType, 0, 1]);
            // }
            // else {
            ...
            //     jobsUsedUpdateFromTopupResp = await connection.query(jobsUsedUpdateFromTopupSql, [data.userId, data.userType, 1, 1]);
            // }
            ...
            const [jobBookKeepingBookingAmountAndJobDetailsResp] = await connection.query(jobBookKeepingBookingAmountAndJobDetailsSql, [data.userId, data.userType, parseInt(resp.insertId), data.job_number, data.total_amount, data.booking_amount, new Date().toISOString().split('T')[0], "booking amount", "Payment", "credit", data.booking_amount, data.job_details])
            ...
            const [resp3] = await connection.query(invDeleteSql);

            let empDeleteSql = "DELETE FROM employee_book WHERE isDeleted = 1 AND bookingStatus = 0"
            const [resp4] = await connection.query(empDeleteSql);
            ...
            const [resp] = await connection.query(jobUpdateSql);
            ...
            const [resp1] = await connection.query(invStatusSql, [parseInt(data.job_id), parseInt(data.userId), parseInt(data.userType), data.job_number, data.equipments]);
            ...
            const [resp2] = await connection.query(empStatusSql, [parseInt(data.job_id), parseInt(data.userId), parseInt(data.userType), data.job_number, data.emp_id]);
