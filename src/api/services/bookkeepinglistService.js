...
exports.getAllBookkeepingList = async (data) => {
    try {
        ...
        if (data.startDate && data.endDate) {
            let sql = "SELECT b_id,book_date,b_description,cust_phone_num,CAST(debit_amount AS FLOAT) AS debit_amount,debit_mode,CAST(credit_amount AS FLOAT) AS credit_amount,credit_mode,CAST(closing_balance AS FLOAT) AS closing_balance,CAST(closing_balance AS FLOAT) AS actual_opening_balance,COALESCE(LAG(CAST(closing_balance AS FLOAT)) OVER (ORDER BY book_date), CAST(closing_balance AS FLOAT)) AS opening_balance,remarks FROM book_keeping WHERE isDeleted = 0 AND userId = " + data.userId + " AND userType = " + data.userType + " AND book_date BETWEEN '" + data.startDate + "' AND '" + data.endDate + "' " + searchCondition + " ORDER BY b_id DESC LIMIT ? OFFSET ?"
