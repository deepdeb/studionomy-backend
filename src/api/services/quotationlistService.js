...
exports.getQuotationList = async (data) => {
    try {
        ...
        if (data.limit && data.quotationType == 'upcoming') {
            let sql = "SELECT q.quotation_id, q.job_details, q.quotation_number, q.studio_name, DATE_FORMAT(q.job_startDate, '%d-%m-%y') as job_startDate, DATE_FORMAT(q.job_endDate, '%d-%m-%y') as job_endDate, q.cust_firstName, q.cust_lastName, q.cust_phoneNo, q.cust_altPhoneNo, q.cust_email, q.address, q.event_location, q.total_amount, q.project_desc, q.customName, q.customValue, q.deliverables, q.termscondition, GROUP_CONCAT(DATE_FORMAT(qb.bookingDate, '%d-%m-%y')) AS bookingDate, GROUP_CONCAT(qb.specialization) AS specialization, GROUP_CONCAT(qb.crew) AS crew FROM quotations q LEFT JOIN quotation_book qb ON q.quotation_id = qb.quotation_id WHERE q.userId = ? AND q.userType = ? AND q.isDeleted = 0 AND q.job_startDate >= '" + today + "' " + searchCondition + " GROUP BY q.quotation_id ORDER BY q.quotation_id DESC LIMIT ? OFFSET ?"
