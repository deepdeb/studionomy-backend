...
exports.getAllJobBookkeepingList = async (data) => {
    try {
        ...
        if(data.payment_criteria) {
            searchCondition += " AND (jbk.job_number LIKE '%" + data.payment_criteria + "%' OR j.cust_phoneNo LIKE '%" + data.payment_criteria + "%' OR jbk.payment_description LIKE '%" + data.payment_criteria + "%' OR jbk.job_value LIKE '%" + data.payment_criteria + "%' OR jbk.job_details LIKE '%" + data.payment_criteria + "%' OR jbk.payment_type LIKE '%" + data.payment_criteria + "%' OR jbk.payment_status LIKE '%" + data.payment_criteria + "%')"
        }
        ...
        logger.error('get all bookkeeping list controller error:', err);
        console.log('get all bookkeeping list controller error:', err);
        return false;
    }
}