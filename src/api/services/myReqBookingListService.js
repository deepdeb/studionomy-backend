...
exports.myRequestBookingList = async (data) => {
    try {
        ...
        let sql = "SELECT u.userId, u.userType, u.orgName, u.location, u.address, u2.name AS freelancerName, u2.mobile AS freelancerMobile, u.mobile AS studioMobile, u.alt_mobile AS studioAltMobile, r.req_id, r.job_id, r.job_number, r.event_location, r.payment, r.skills, r.equipment_id, i.inv_code, r.message, DATE_FORMAT(r.req_date, '%Y-%m-%d') AS req_date, r.req_to, r.req_to_userType, r.req_status, DATE_FORMAT(r.update_date, '%Y-%m-%d') AS update_date FROM request_for_booking AS r JOIN users AS u ON r.req_from = u.userId LEFT JOIN users AS u2 ON r.req_to = u2.userId JOIN inventary i ON r.equipment_id = i.inv_id WHERE r.isDeleted = 0 AND r.isCancel = 0 AND r.req_from = ? AND r.req_from_userType = ?"
